 ...
 */
function checkLocalTabMove(key: string, tabbar: HTMLDivElement | null): boolean {
...
export function DockTabBar(props: DockTabBarProps) {
  ...
  const getRef = (div: HTMLDivElement | null) => {
    ref.current = div;
  };

  const onKeyDown = (e: React.KeyboardEvent) => {
    if (e.key.startsWith("Arrow")) {
      if (ref.current && !checkLocalTabMove(e.key, ref.current) && !isMaximized) {

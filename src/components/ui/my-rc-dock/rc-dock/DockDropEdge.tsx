...
export class DockDropEdge extends React.PureComponent<DockDropEdgeProps, any> {
  ...
  getDirection(
    ...
    samePanel: boolean,
    tabLength: number
  ): { direction: DropDirection | null; mode?: DockMode; depth: number } {
    if (!this._ref) {
  ...
  getActualDepth(
    ...
    let targetBox: BoxData | undefined = panelData.parent;
    let lastDepth = 0;
    if (targetBox && targetBox.mode === mode) {
      ++depth;
    }
  ...
  onDragOver = (e: DragState) => {
    ...
    let { direction, mode, depth } = this.getDirection(
      e,
      fromGroup,
    ...
    if (direction === "float" && dropFromPanel.panelLock) {
      this.context.setDropRect(null, "remove", this);
      return;
    }
    let targetElement: HTMLElement | null = panelElement;
    for (let i = 0; i < depth && targetElement; ++i) {

...
export interface PanelBase {
  ...
  /**
   * float mode only: when true, panel is minimized to bottom (tab bar only).
   * Restore uses _floatRestoreY / _floatRestoreH if set.
   */
  minimized?: boolean;
  /** @ignore float minimize restore */
  _floatRestoreY?: number;
  /** @ignore float minimize restore */
  _floatRestoreH?: number;
  /** @ignore 최소화 전 최대화 상태였는지 (복원 시 다시 최대화) */
  _wasMaximized?: boolean;
...
export type DropDirection =
  ...
  | "front"
  | "maximize"
  | "minimize" // float panel: minimize to bottom (tab bar only)
  | "restore" // float panel: restore from minimized
  | "new-window"

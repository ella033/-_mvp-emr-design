...
function toDateValue(date: Date | string | undefined): string {
  if (!date) return "";
  const d = typeof date === "string" ? new Date(date) : date;
  return isNaN(d.getTime()) ? "" : formatDateToYyyyMmDdHyphen(d);
}
import {
  Tooltip,
  TooltipContent,
...
const SecondMedical: React.FC<SecondMedicalProps> = ({
  ...
  const [selectedInsuranceType, setSelectedInsuranceType] = useState<string>(
    String(currentInsuranceInfo?.uDeptDetail || "")
  );
  ...
  useEffect(() => {
    if (!currentInsuranceInfo) {
      ...
      setSelectedInsuranceType(String(currentInsuranceInfo?.uDeptDetail || ""));
    }
  }, [currentInsuranceInfo]);
  ...
  // 지역/직장: 입력용 로컬 상태 (InsuranceInfo에 해당 필드 없음, UI 전용)
  const [local지역직장, setLocal지역직장] = useState("");
      ...
      >
        ...
        <div className="col-span-2 w-full border border-gray-300 rounded p-1 pl-2 text-sm text-gray-900 bg-[var(--bg-3)]">
          {currentInsuranceInfo?.fatherRrn
            ? `${currentInsuranceInfo.fatherRrn.slice(0, 6)}-${currentInsuranceInfo.fatherRrn.slice(6, 13)}`
            : "000000-0000000"}
        </div>
        ...
        <input
          type="text"
          value={currentInsuranceInfo?.차상위특정기호 ?? ""}
          onChange={(e) => {
            if (document.activeElement === e.target) {
              onInsuranceDataChange?.({ 차상위특정기호: e.target.value });
            }
          }}
          onBlur={(e) => {
            onInsuranceDataChange?.({ 차상위특정기호: e.target.value });
        ...
        <input
          type="text"
          value={local지역직장}
          onChange={(e) => setLocal지역직장(e.target.value)}
          placeholder="-"
          disabled={isDisabled}
          className={`w-full border border-[var(--border-2)] rounded p-1 pl-2 text-sm text-[var(--main-color)] focus:outline-none focus:border-blue-500 ${isDisabled ? "bg-[var(--bg-3)]" : "bg-[var(--bg-main)]"}`}
        />
        ...
        <input
          type="date"
          value={toDateValue(currentInsuranceInfo?.차상위승인일)}
          onChange={(e) => {
            if (document.activeElement === e.target && e.target.value) {
              onInsuranceDataChange?.({
                차상위승인일: new Date(e.target.value),
              });
            }
          }}
          onBlur={(e) => {
        ...
        <input
          type="date"
          value={toDateValue(currentInsuranceInfo?.차상위종료일)}
          onChange={(e) => {
            if (document.activeElement === e.target && e.target.value) {
              onInsuranceDataChange?.({
                차상위종료일: new Date(e.target.value),
              });
            }
          }}
          onBlur={(e) => {

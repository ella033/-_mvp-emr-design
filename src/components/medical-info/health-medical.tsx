...
const HealthMedical: React.FC<HealthMedicalProps> = ({
  ...
  /*
    const updateChronic = useCallback(
      (is만성질환관리: boolean) => {
        const { openedReceptions, openedReceptionId } = useReceptionStore.getState();
        const currentReception = openedReceptions.find(
          (r) => r.originalRegistrationId === openedReceptionId
        );
  
        if (!currentReception?.insuranceInfo?.disRegClass) {
          updateInsuranceInfo({ is만성질환관리 });
          return;
        }
  
        // disRegClass 인스턴스 직접 수정
        const disRegClass = currentReception.insuranceInfo.disRegClass;
  
        // chronicCur 업데이트
        disRegClass.chronicCur.isData = is만성질환관리;
  
        // chronicIntegrated도 함께 업데이트 (만약 만성질환관리가 false면 통합도 false)
        if (!is만성질환관리) {
          disRegClass.chronicIntegrated.isData = false;
        }
  ...
  // 접수일(selectedDate)에 따라 표시만 계산. 이 값으로 저장하지 않음(체크/input 변경 시에만 저장).
  ...
  /*
    const handleChronicChange = (isChronic: boolean) => {
      updateChronic(isChronic);
        ...
        <div className="col-span-3 flex items-center gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-1">
            <input
              type="checkbox"
              id="chronic"
              checked={currentInsuranceInfo?.is만성질환관리 || false}
              onChange={(e) => {
                if (document.activeElement === e.target) {
                  handleChronicChange(e.target.checked);
                }
              }}
              onFocus={(e) => {
                e.target.setAttribute(
                  "data-previous-value",
                  String(currentInsuranceInfo?.is만성질환관리 || false)
                );
              }}
              onBlur={(e) => {
                const previousValue = e.target.getAttribute(
                ...
                const currentValue = String(
                  currentInsuranceInfo?.is만성질환관리 || false
                );
                if (previousValue !== currentValue) {
                  handleChronicChange(
                    currentInsuranceInfo?.is만성질환관리 || false

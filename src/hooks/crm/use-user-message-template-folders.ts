import { useQuery } from "@tanstack/react-query";
import { CrmUserMessageTemplateService } from "@/services/crm-user-message-template-service";
import type { FoldersResponse } from "@/types/crm/message-template/crm-user-message-template-types";
import { CrmMessageType } from "@/constants/crm-enums";

export function useUserMessageTemplateFolders(messageType: number) {
  // messageType이 유효한 값(1: 문자, 2: 알림톡)인지 확인
  const isValidMessageType = messageType === CrmMessageType.문자 || messageType === CrmMessageType.알림톡;
  
  return useQuery({
    queryKey: ["crm-user-message-template-folders", messageType],
    queryFn: async (): Promise<FoldersResponse> => {
      return await CrmUserMessageTemplateService.getFolders(messageType);
    },
    staleTime: 10 * 60 * 1000,
    enabled: isValidMessageType,
  });
}

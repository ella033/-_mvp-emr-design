...
import {
  pciToUnified,
  selfCheckToUnified,
} from "@/types/chart-check/chart-check-types";

export default function ChartCheck() {
  const {
    selectedEncounter,
    pciCheckResults,
    clearPciCheckResults,
    chartCheckResults,
    clearChartCheckResults,
  } = useEncounterStore();

  // PCI + 자체점검 병합
  const unifiedItems = useMemo(() => {
    const pciItems = pciCheckResults.map(pciToUnified);
    const selfItems = chartCheckResults.map(selfCheckToUnified);
    return [...pciItems, ...selfItems];
  }, [pciCheckResults, chartCheckResults]);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden text-[var(--text-primary)] border border-[var(--border-1)] rounded-sm">
      {/* 헤더 */}

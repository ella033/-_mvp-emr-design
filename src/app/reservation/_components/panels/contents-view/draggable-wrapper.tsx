...
export default function DraggableWrapper({
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent) => {
    e.preventDefault();
    if (contextMenuItems.length === 0 || disabled) return;
  ...
  }, [contextMenuItems.length, disabled]);
  ...
  return (
    <>
      ...
      {/* Drag Overlay */}
      {dragState.isDragging && showOverlay && dragState.currentPos && dragState.offset && (
        <DragOverlay
...
function DragOverlay({ position, offset, children }: DragOverlayProps) {
  return (
    <div
    ...
    >
      <div className="shadow-2xl rounded-lg border-2 border-blue-400 bg-white opacity-90 transform scale-105 text-gray-900">
        {children}
      </div>
    </div>
  );

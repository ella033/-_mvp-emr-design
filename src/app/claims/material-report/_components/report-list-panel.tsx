...
import type { MaterialReportListItem } from "@/types/claims/material-report";
import { StatusBadge } from "./status-badge";

type ReportListPanelProps = {
  reports: MaterialReportListItem[];
  ...
  return (
    <div className="flex h-full w-[420px] flex-col gap-3 border-r border-border bg-white p-3 shadow-sm">
      <div className="flex items-center gap-4">
        <span className="text-[14px] font-semibold text-[#171719]">조회 기간</span>
        <KstDateRangeCalendarInput
          startUtcIso={startDate}
          endUtcIso={endDate}
          onChange={(next) => {
            if (next.startUtcIso) onDateRangeChange("startDate", next.startUtcIso);
            if (next.endUtcIso) onDateRangeChange("endDate", next.endUtcIso);
          }}
          className="gap-2"
        ...
        <Table>
          <TableHeader className="sticky top-0 z-10">
            <TableRow className="bg-[#F4F4F5] hover:bg-[#F4F4F5] h-[28px] border-none">
              ...
              <TableHead className="w-[110px] text-[12px] font-semibold text-[#292A2D] text-center px-2">작성일자</TableHead>
              <TableHead className="w-[120px] text-[12px] font-semibold text-[#292A2D] text-center px-2">신청번호</TableHead>
              <TableHead className="w-[60px] text-[12px] font-semibold text-[#292A2D] text-center px-2">
                건수
              </TableHead>
              <TableHead className="w-[100px] text-[12px] font-semibold text-[#292A2D] text-center px-2">송신상태</TableHead>
              <TableHead className="text-[12px] font-semibold text-[#292A2D] text-center px-2">메모</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading && (
              <TableRow className="border-none">
                <TableCell colSpan={6} className="py-10 text-center text-[13px] text-[#989BA2]">
                  목록을 불러오는 중입니다.
                </TableCell>
              </TableRow>
            )}
